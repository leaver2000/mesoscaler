...
@pytest.mark.parametrize(
    "enum,dx,dy,height,width,time_step,levels,area_of_interest",
    [
...
def test_resampler(
    ...
    producer = ms.create.producer(
        dsets,
        ms.AreaOfInterestSampler.partial(aoi=area_of_interest, time_step=time_step),
        height=height,
        width=width,
        dx=dx,
        dy=dy,
    ...
    for idx in random.choices(producer.indices, k=5):
        ...
        assert x.shape == (len(members), time_step, len(levels), height, width)
        for j, member in enumerate(members, 1):
            v = num * j
            assert np.all(x[j - 1] == v), f"{idx[0]} expected {v} got {np.unique(x[j - 1])}"
