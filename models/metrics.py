...
from ..datasets.enso.core import NINO_WINDOW_T, scale_back_sst


# =====================================================================================================================
# enso
# =====================================================================================================================
def compute_enso_score(y_pred, y_true, acc_weight: Optional[Union[str, np.ndarray, torch.Tensor]] = None):
    r"""
    ...
    if acc_weight is None:
        ...
        if acc_weight == "default":
            acc_weight = torch.tensor([1.5] * 4 + [2] * 7 + [3] * 7 + [4] * (nino_out_len - 18))[
                :nino_out_len
...
def sst_to_nino(sst: torch.Tensor, normalize_sst: bool = True, detach: bool = True):
    r"""
...
class ENSOScore(Metric):
    def __init__(self, layout="NTHW", out_len=26, normalize_sst=True):
    ...
    def update(
        ...
        preds: torch.Tensor,
        target: torch.Tensor,
        nino_preds: torch.Tensor | None = None,
        nino_target: torch.Tensor | None = None,
    ) -> None:
        r"""
        Parameters
        ----------
        ...
        if nino_preds is None:
            nino_preds = sst_to_nino(sst=preds, normalize_sst=self.normalize_sst)
        if nino_target is None:
            nino_target = sst_to_nino(sst=target, normalize_sst=self.normalize_sst)
